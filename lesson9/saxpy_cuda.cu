...
#include "cstdio" 
/* using atribute noinline will preserve the function so it shows up in cpu profiling */
__attribute__((noinline))  void saxpy_cpu(float *vecY, float *vecX, float alpha, int n) {
    int i;
...
int main(int argc, char *argv[]) {
    ...
    int n = 64*1024*1024; /* make n large for meaningful profiling */
    float alpha = 0.5f;
    int nerror;
    ...
    printf("Working on vector size n=%d\n",n);
