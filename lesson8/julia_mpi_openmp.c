...
This code is exactly the same as the code in julia_mpi.c except that one OpenMP pragma line has been added.

Compile with:

mpicc -fopenmp -O2 julia_mpi_openmp.c

Example run:

OMP_NUM_THREADS=2 OMP_SCHEDULE=guided  OMP_PROC_BIND=true mpirun -bind-to-core -np 12 ./a.out
